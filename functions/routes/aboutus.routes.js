...
router.get("/aboutus", async(req, res) => {
   var data = await getDataCollection('AboutUs');
   res.json({data});
});

router.get("/aboutus/:id", async(req, res) => {
   console.log(req.params.id);
    var data = await getDataCollectionForId('AboutUs', req.params.id);
    res.json({data});
 });
...
/* router.post("/aboutus", async(req, res) => {
 ...
 router.patch("/aboutus", async(req, res) => {
    var dataUpdate = await updateDataCollection('AboutUs', req.body.id, req.body);
