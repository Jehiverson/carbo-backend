...
router.get("/location", async(req, res) => {
   var data = await getDataCollection('Location');
   res.json({data});
});

router.get("/location/:id", async(req, res) => {
   console.log(req.params.id);
    var data = await getDataCollectionForId('Location', req.params.id);
    res.json({data});
 });
...
/* router.post("/aboutus", async(req, res) => {
 ...
 router.patch("/location", async(req, res) => {
    var dataUpdate = await updateDataCollection('Location', req.body.id, req.body);
