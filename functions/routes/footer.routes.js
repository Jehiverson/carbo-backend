...
router.get("/footer", async(req, res) => {
   var data = await getDataCollection('Footer');
   res.json({data});
});

router.get("/footer/:id", async(req, res) => {
   console.log(req.params.id);
    var data = await getDataCollectionForId('Footer', req.params.id);
    res.json({data});
 });
...
/* router.post("/aboutus", async(req, res) => {
 ...
 router.patch("/footer", async(req, res) => {
    var dataUpdate = await updateDataCollection('Footer', req.body.id, req.body);
